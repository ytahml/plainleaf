...
- A real `WKWebView` loads the generated document and produces a non-blank snapshot.
...
- Footnote numbers remain visually subordinate and keyboard-focusable; selecting a reference moves to its endnote, each return link moves to the exact occurrence, and both Paper and Ink remain readable.
...
- `Shift-Command-E` opens a save panel with an `.html` name; the resulting single file loads offline with the current typography, semantic content, local images, footnotes, and no remote resources or executable raw HTML.
